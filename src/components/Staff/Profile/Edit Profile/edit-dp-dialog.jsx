...
const EditDpDialog = ({ defaultData }) => {
  const [src, setSrc] = useState(defaultData?.OfficeStaffInfo?.avtar_url);
  // const [value, setValue] = useState(null);
  const [open, setOpen] = useState(false);

  const { register, handleSubmit, setValue } = useForm();

  const mutation = useUpdateStaffDP(defaultData.id);
  ...
  const handleClear = () => {
    ...
    setSrc(defaultData?.OfficeStaffInfo?.avtar_url);
  };
        ...
        <div>
          <div className="relative w-max mx-auto">
            <Avatar className="h-44 w-44 ">
              <AvatarImage src={src} alt={defaultData?.OfficeStaffInfo?.name} className=" object-cover object-center" />
              <AvatarFallback>
                {nickName(defaultData?.OfficeStaffInfo?.name)}
              </AvatarFallback>
            </Avatar>
            <div
              className={`absolute top-2 right-2 bg-background rounded-full ${
                src === defaultData?.OfficeStaffInfo?.avtar_url ? "hidden" : ""
              }`}
            >
              <Button
                variant="ghost"
          ...
          <form action="" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="mt-3 flex justify-end gap-2">
              ...
              <Button
                disabled={
                  src === defaultData?.OfficeStaffInfo?.avtar_url ||
                  mutation.isPending
                }
              >
                {mutation.isPending ? (
